...
class ICarl(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        self.nota = args.dataset.rsplit('-',1)[-1]=='nota'

        # Instantiate buffers
        self.buffer = Buffer(self.args.buffer_size, self.device)
        if (self.nota):
            self.eye = torch.eye(self.dataset.N_CLASSES_PER_TASK *
                                self.dataset.N_TASKS + 1).to(self.device)      
        else:      
            self.eye = torch.eye(self.dataset.N_CLASSES_PER_TASK *
    ...
    def get_loss(self, inputs: torch.Tensor, labels: torch.Tensor,
        ...
        if self.nota:
            ind_slice = np.r_[:ac, len(self.eye)-1]
        else:
            ind_slice = np.r_[:ac]

        outputs = self.net(inputs)[:, ind_slice]
        ...
        if task_idx == 0:
            ...
            if self.nota:
                ...
                ind_slice = np.r_[:ac]
            targets = self.eye[labels][:, ind_slice]
            loss = F.binary_cross_entropy_with_logits(outputs, targets)
            assert loss >= 0
        else:
            if self.nota:
                ind_slice = np.r_[pc:ac, len(self.eye)-1]
            else:
                ind_slice = np.r_[pc:ac]
            targets = self.eye[labels][:, ind_slice]
    ...
    def begin_task(self, dataset):
        ...
        if self.current_task > 0:
            ...
            if type(dataset.train_loader.dataset.data) == torch.Tensor:
                ...
                A = np.expand_dims(dataset.train_loader.dataset.data,axis=1).swapaxes(1, 3)
                B = torch.stack([(denorm(self.buffer.examples[i] * 255).type(torch.uint8).cpu())
