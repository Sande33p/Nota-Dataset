...
def print_mean_accuracy(mean_acc: np.ndarray, task_number: int,
    ...
    if setting == 'domain-il':
        ...
        try:
            mean_acc_class_il, mean_acc_task_il, mean_acc_nota, mean_acc_nota_til = mean_acc
            print('\nAccuracy for {} task(s): \t [Class-IL]: {} %'
                ' \t [Task-IL]: {} %'
                ' \t [Task-IL-nota]: {} %'
                ' \t [Class-IL-nota]: {} %\n'.format(task_number, round(mean_acc_class_il, 2), round(mean_acc_task_il, 2),round(mean_acc_nota_til, 2),round(mean_acc_nota, 2)), file=sys.stderr)
        except:
...
class CsvLogger:
    def __init__(self, setting_str: str, dataset_str: str,
                 model_str: str, num_tasks: int) -> None:
        self.accs = []
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        path = base_path() + "results/" + self.setting + "/" + self.dataset\
               + "/" + self.model + "/mean_accs_"+str(self.num_tasks)+".csv"
        print ("path_csv", path)
